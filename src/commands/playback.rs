...
use lavalink_rs::{
	...
	model::track::{PlaylistData, Track, TrackData},
	player_context::{PlayerContext, TrackInQueue},
	prelude::TrackLoadData,
};
...
// Functions
/// Converts the load response from Lavalink to a simple list of tracks.
///
/// This silently discards errors.
fn track_load_response_to_track_list(track_load_response: Track) -> Vec<TrackData> {
	if let Some(track_load_data) = track_load_response.data {
		match track_load_data {
			TrackLoadData::Track(track_data) => vec![track_data],
			TrackLoadData::Playlist(PlaylistData { tracks, .. }) => tracks,
			TrackLoadData::Search(search_results) => search_results,
			TrackLoadData::Error(_) => Vec::new(),
		}
	} else {
		Vec::new()
	}
...
fn build_track_user_data(requestor_id: UserId) -> Option<JsonValue> {
	Some(json!({"requestor_id": requestor_id.get()}))
}

fn get_user_data_requestor_id(user_data: Option<JsonValue>) -> UserId {
	UserId::new(
		user_data.expect("user data should be set")["requestor_id"]
			.as_u64()
			.expect("the requestor ID should always be valid as a u64 type"),
	)
}

async fn join_internal(ctx: PoiseContext<'_>, announce_success: bool) -> Result<(), ()> {
	let Some(guild) = ctx.guild().as_deref().map(ToOwned::to_owned) else {
	...
	let join_result = ctx.data().songbird.join_gateway(guild.id, channel_id).await;
	match join_result {
		Ok((connection_info, _)) => {
			if let Err(e) = lavalink_client
				.create_player_context(guild.id, connection_info)
				.await
			{
				reply(
					ctx,
...
/// Otherwise, it will search the query on YouTube and queue up the first
/// result. Age-restricted videos likely won't work.
///
...
async fn get_player_context_or_join(
	ctx: PoiseContext<'_>,
	...
	if let Some(player_context) = lavalink_client.get_player_context(guild_id) {
		...
		// Join if not already in a call
		if join_internal(ctx, false).await.is_err() {
			return Err(());
		}

		Ok(lavalink_client.get_player_context(guild_id).unwrap())
	}
}
...
async fn play_internal(ctx: PoiseContext<'_>, query: &str, shuffle: bool) -> Result<(), Error> {
	...
	let lavalink_client = &ctx.data().lavalink;
	let Ok(player_context) = get_player_context_or_join(ctx, lavalink_client, guild_id).await
	else {
		return Ok(());
	};
	...
	if let PoiseContext::Prefix(prefix_ctx) = ctx {
		for attachment in &prefix_ctx.msg.attachments {
			...
			// Queue it up
			let track_load_response = lavalink_client
				.load_tracks(guild_id, &attachment.url)
				.await?;
			let mut track_list = track_load_response_to_track_list(track_load_response);
	...
	let track_load_response = lavalink_client.load_tracks(guild_id, query_trimmed).await?;
	let track_list = track_load_response_to_track_list(track_load_response);
	...
	let desired_track_count = if is_url { track_list.len() } else { 1 };

	queueable_tracks.extend(track_list.iter().take(desired_track_count).cloned());
	...
	if is_url && desired_track_count == 1 {
		let track_info = &mut queueable_tracks[0].info;
	...
	if desired_track_count > 1 && shuffle {
		let mut rng = thread_rng();
		let mut inverse_riffle_shuffler = Irs::default();
		inverse_riffle_shuffler
			.shuffle(&mut queueable_tracks, &mut rng)
			.ok();
		// Ignore the error here because if the shuffle fails
		// (which it never should) we want to continue
	}
	...
	for (index, track) in queueable_tracks.iter().enumerate() {
		...
		let track_identifier = &track.info.identifier;
		...
		'sponsor_block: {
			...
			/// The maximum difference from the submission video length to
			/// accept.
			...
			let Some(track_uri) = &track_info.uri else {
				break 'sponsor_block;
			};
			let parsed_uri = Url::parse(track_uri).expect(
				"unable to parse track info URI when it should have been guaranteed to be valid",
			);
			...
			if let Some(video_id) = get_youtube_video_id(&parsed_uri) {
					...
					.await
				{
					...
					let track_duration = track_info.length as f32 / MILLIS_PER_SECOND_F32;
					...
					if skip_timecodes_len > 0 {
						...
						// track
						if index == 0 {
							let new_track_duration = track_info.length
							...
							// of <1s
							if new_track_duration <= track_info.length - MILLIS_PER_SECOND {
						...
						if skip_timecodes[0].start < TRACK_ENDING_IMPRECISION {
							...
							new_start_time = Some(Duration::from_secs_f32(skip_timecodes[0].end));
						}
					...
					// Cache the segments if there are segments to cache
		...
		// If no segments were found, cache that fact so that we don't have to check the
		// next time the video is requested
		...
		// Queue
		let mut queueable = TrackInQueue::from(track.clone());
		queueable.start_time = new_start_time;
		queueable.track.user_data = build_track_user_data(ctx.author().id);

		if let Err(e) = player_context.queue(queueable) {
	...
	if queueable_tracks_len == 1 {
		let track_info = &queueable_tracks[0].info;
		reply(
			ctx,
			format!(
				"Added to queue: {} [{}]",
		...
		reply_embed(
			ctx,
			CreateEmbed::new()
				.title(format!("Added {queueable_tracks_len} Tracks:"))
				.description(desc),
		)
		.await?;
	}
...
pub async fn tts(
	...
	async fn queue_fragment(
		...
		// Fetch the fragment
		let tts_query = format!("speak:{fragment}");
		let track_load_response = lavalink_client
			.load_tracks(guild_id, tts_query.as_str())
			.await?;
		let track_list = track_load_response_to_track_list(track_load_response);
		...
		// Queue it
		let mut queueable = TrackInQueue::from(track_list[0].clone());
		queueable.track.user_data = build_track_user_data(ctx.author().id);
	...
	for word in message_trimmed.split_whitespace() {
		...
		if message_buffer.len() + word.len() > MAX_SINGLE_FRAGMENT_SIZE {
			queue_fragment(ctx, guild_id, message_buffer.trim_end()).await?;
		...
		message_buffer.push(' '); // Technically this isn't accounted for above, but
		                    // we have a buffer between
		                    // our fragment limit and the
		                    // actual limit anyway
	}
...
pub async fn shuffle(ctx: PoiseContext<'_>) -> Result<(), Error> {
	...
	let queue_reference = player_context.get_queue();
	let Ok(mut old_queue) = queue_reference.get_queue().await else {
		reply(ctx, "Failed to get the current queue.").await?;
		return Ok(());
	};
	...
	if old_queue.len() > 1 {
		...
		let first_track = old_queue.pop_front().expect("the queue is not empty");
		let mut shuffle_list = Vec::from(old_queue);

		{
			let mut rng = thread_rng();
			let mut inverse_riffle_shuffler = Irs::default();
			inverse_riffle_shuffler
				.shuffle(&mut shuffle_list, &mut rng)
				.ok();
		}

		let mut new_queue = VecDeque::from(shuffle_list);
		new_queue.push_front(first_track);

		if queue_reference.replace(new_queue).is_err() {
			reply(ctx, "Failed to set the new (shuffled) queue.").await?;
			return Ok(());
		};
	}
...
pub async fn seek(
	...
	if let Err(e) = player_context.set_position(time_dur).await {
...
pub async fn clear(ctx: PoiseContext<'_>) -> Result<(), Error> {
	...
	if player_context.get_queue().clear().is_err() {
		reply(ctx, "Failed to clear the queue.").await?;
		return Ok(());
	}
...
pub async fn now_playing(ctx: PoiseContext<'_>) -> Result<(), Error> {
	...
	let Ok(player_data) = player_context.get_player().await else {
		reply(ctx, "Unable to get the player data.").await?;
		return Ok(());
	};

	if let Some(now_playing) = player_data.track {
		let track_info = now_playing.info;
		let track_has_url_source = track_info
			.uri
			.as_ref()
			.is_some_and(|uri| uri_is_url(uri.as_str()));
		let track_segments = {
			let mut segment_data_handle = ctx.data().segment_data.lock().unwrap();
			segment_data_handle
				.cached_segments
		...
		let mut embed = CreateEmbed::new()
			...
			.field(
				"Requested By:",
				get_user_data_requestor_id(now_playing.user_data)
					.mention()
					.to_string(),
				false,
			);
...
pub async fn queue(ctx: PoiseContext<'_>) -> Result<(), Error> {
	...
	if queue_len > 0 {
		...
		for (i, queued_track) in queue.iter().enumerate() {
			let track_info = &queued_track.track.info;
